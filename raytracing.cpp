...
void threadFunc(Scene &scene, ViewPlane &viewPlane, Image &image, int sizeX, int sizeY, int numOfSamples, int startY, int endY) {
...
int main(int argc, char **argv) {
    ...
    int partitionY = viewPlaneResolutionY/numberOfThreads;
    for (int i = 0; i < numberOfThreads; i++) {
        int startY=partitionY*i;
        int endY=partitionY*(i+1);
        thread thr(threadFunc, ref(scene), ref(viewPlane), ref(image), 
            viewPlaneResolutionX, viewPlaneResolutionY, numOfSamples, startY, endY);
    ...
    cout << "Time = " << time << endl << "SizeX = " << viewPlaneResolutionX << endl << "SizeY = " << viewPlaneResolutionY << endl << "PartitionY = " << partitionY << endl << "Threads count = " << numberOfThreads << endl;
