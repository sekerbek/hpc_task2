...
void threadFunc(Scene &scene, ViewPlane &viewPlane, Image &image, int sizeX, int sizeY, int numOfSamples,int block_size,mutex &mut,queue<int> &data) {
            while (true) {
                double piece;
                {
		...
		int start = piece*block_size;
        	int end = (piece+1)*block_size;
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < numberOfThreads; i++) {
        thread thr(threadFunc, ref(scene), ref(viewPlane), ref(image), 
            viewPlaneResolutionX, viewPlaneResolutionY, numOfSamples,block_size,ref(mut),ref(data));
    ...
    image.saveJPEG("raytracing_dynamic_" + to_string(numberOfThreads) + ".jpg");
